<x-app-layout title="Manajemen Role">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900">Role</h1>
            <p class="text-gray-600">Kelola role dan izin akses untuk tim Anda.</p>
        </div>
        <a href="{{ route('roles.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-blue-700">
            ➕
    ...
    <div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-100 bg-white">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-600">
                            <div class="flex flex-wrap gap-2">
                                @forelse ($role->permissions as $permission)
                                    <span class="rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700">{{ $permission->name }}</span>
                                @empty
                                    <span class="text-gray-400">Belum ada izin</span>
                                @endforelse
                            </div>
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-600">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('roles.edit', $role) }}" class="rounded-lg border border-blue-200 px-3 py-1 text-xs font-semibold text-blue-700 hover:bg-blue-50">Edit</a>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-4 text-center text-sm text-gray-500">Belum ada data role.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
