<x-app-layout title="Tambah User">
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-gray-600">Buat akun baru dan atur role yang sesuai.</p>
        </div>
    ...
    <form action="{{ route('users.store') }}" method="POST" class="space-y-6">
        ...
        <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm space-y-4">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" required>
                    @error('email')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input type="password" name="password" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" required>
                    @error('password')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="password" name="password_confirmation" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none" required>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
                @foreach ($roles as $role)
                    <label class="flex items-start gap-3 rounded-lg border border-gray-200 p-3 hover:bg-gray-50">
                        <input type="checkbox" name="roles[]" value="{{ $role->id }}" class="mt-1 h-4 w-4 rounded border-gray-300 text-blue-600" {{ in_array($role->id, old('roles', [])) ? 'checked' : '' }}>
                        <div>
        ...
        <div class="flex justify-end">
            <button type="submit" class="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-5 py-2 text-sm font-semibold text-white shadow hover:bg-blue-700">Simpan User</button>
        </div>
    </form>
</x-app-layout>
