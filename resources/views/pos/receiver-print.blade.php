<x-app-layout title="Cetak Penerima POS">
    <style>
        @media print {
            .print-actions {
    ...
    <div class="mx-auto max-w-3xl space-y-6">
        <div class="flex flex-wrap items-center justify-between gap-4 print-actions">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900">Cetak Penerima</h1>
                <p class="text-gray-600">Label pengiriman untuk transaksi {{ $transaction->invoice_number }}.</p>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                <a href="{{ route('pos.receipt', $transaction) }}" class="rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-700 shadow-sm hover:bg-gray-50">Kembali</a>
        ...
        <div class="print-card rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div class="space-y-3">
                    ...
                    <p class="text-lg font-semibold text-gray-900">{{ $transaction->customer->name }}</p>
                    <p class="text-sm text-gray-700 whitespace-pre-line">{{ $transaction->customer->address ?? '-' }}</p>
                    <p class="text-sm text-gray-700">Telp: {{ $transaction->customer->phone ?? '-' }}</p>
                </div>
            </div>
        </div>
    </div>
