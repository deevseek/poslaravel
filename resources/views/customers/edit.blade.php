<x-app-layout title="Edit Customer">
    <div class="mb-6">
        ...
        <p class="text-gray-600">Perbarui informasi customer.</p>
    </div>
        ...
        <form action="{{ route('customers.update', $customer) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <textarea id="address" name="address" rows="3"
                    class="mt-2 w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('address', $customer->address) }}</textarea>
                @error('address')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
